# 多人邀请版部署

这份项目分为两个服务：Vercel 部署网页，Render（或你自己的 Node 主机）部署联机服务器。**只部署 Vercel 还不能联机。** 无需数据库账号或把私密密钥放进网页。

## 1. 上传代码

把项目上传到自己的 Git 仓库，包含 package-lock.json、public、src、multiplayer、scripts、server.mjs、vercel.json 和 render.yaml。不要上传 node_modules、dist、.npm-cache 或 .env 文件。

## 2. 部署联机服务器（Render）

导入同一仓库创建 Web Service，或用仓库内的 render.yaml 创建 Blueprint。

- Runtime：Node，版本 22 或更高
- Build Command：`npm ci`
- Start Command：`npm start`
- 环境变量 `HOST`：`0.0.0.0`
- 环境变量 `ALLOWED_ORIGINS`：你的 Vercel 正式网址，例如 `https://shadow-protocol.vercel.app`。多个网址用英文逗号分隔，不要末尾斜杠。
- PORT 使用 Render 自动设置的值，不需要手动设置。
- 保持单个实例。房间暂存在实例内存里，不能横向扩容到多个实例。

记录服务地址，例如 `https://shadow-server.onrender.com`。

## 3. 部署网页（Vercel）

导入同一仓库，Framework Preset 选择 Other：

- Build Command：`npm run build`
- Output Directory：`dist`
- 环境变量 `PUBLIC_GAME_SERVER_URL`：`wss://shadow-server.onrender.com/multiplayer`（替换成第 2 步的实际地址）。

vercel.json 已包含构建设置。环境变量修改后需要重新部署。Vercel 正式域名生成后，回到 Render 把实际域名填进 ALLOWED_ORIGINS 并重启服务。Preview 部署域名若也要联机，需单独加入允许名单。

## 4. 邀请朋友

打开 Vercel 网址 → 输入昵称 → 创建房间 → 复制邀请链接 → 发给朋友。朋友打开链接后输入昵称，点“加入房间”。也可手动输入六位房间码。

4–8 人全部准备后，房主开始游戏。J 转换、K 攻击；任务需完成面板小游戏。每个人只能控制自己的角色。大厅、游戏和会议均提供“通讯 · 文字 / 语音”面板。

## 文字聊天与语音

- 输入消息后按 Enter 或点击发送；每条最多 300 字符，发送间隔至少 0.7 秒。中文输入法确认候选字不会发送消息。打字时角色停止移动。
- 大厅和结算后全员交流；对局中存活玩家与淘汰玩家分开交流，探索和会议都可使用。切换频道时清空本地聊天记录。记录不保存到数据库，刷新和重连会清空。
- 点击“加入语音”才申请麦克风权限。提供静音、取消静音、退出语音和参与者状态。“音效”开关不影响语音。浏览器阻止播放时点击“播放语音”。
- 离开房间、断线或刷新会停止麦克风；重连后需主动重新加入语音。语音失败不影响文字聊天。
- 正式网页必须使用 HTTPS，联机地址使用 WSS；本地可用 localhost / 127.0.0.1。语音使用浏览器 WebRTC，小房间内点对点连接；不录音。

### 配置跨网络语音中继

默认使用 `stun:stun.l.google.com:19302`，只在加入语音后建立连接。可在 **Node 服务器**设置 `STUN_URLS`（多个地址用逗号分隔；设为空则只尝试直连）。STUN 不能保证所有 NAT、防火墙或移动网络之间互通；可靠公网语音需要可用的 TURN 服务。

支持 coturn 的 REST 临时凭证认证。在同一 Node 服务器设置：

```text
TURN_URLS=turn:YOUR-TURN-HOST:3478?transport=udp,turns:YOUR-TURN-HOST:5349?transport=tcp
TURN_SHARED_SECRET=与 coturn static-auth-secret 相同的服务端密钥
```

需要自行准备 TURN 主机或兼容服务，并正确配置其域名、证书、监听端口和中继端口范围。这里不会创建或购买服务。TURN 需要同时配置地址和密钥；重启 Node 服务生效，无须把密钥放到 Vercel 前端。服务端只向入房并加入语音的玩家下发 12 小时有效的连接凭证，原始共享密钥不会发给浏览器。房间最长存活 6 小时。

频道隔离由服务端信令权限和客户端连接清理共同执行。点对点语音不能阻止恶意修改客户端后利用已有连接串通，不用于竞技反作弊承诺。

浏览器实现依据：[MDN getUserMedia](https://developer.mozilla.org/en-US/docs/Web/API/MediaDevices/getUserMedia)、[WebRTC 连接协商](https://developer.mozilla.org/en-US/docs/Web/API/WebRTC_API/Perfect_negotiation)。本实现用玩家 ID 确定唯一 offer 发起方，避免同时发起协商。

### 可重复的本地音频 QA

运行 `node scripts/communication-preview.mjs`，打开 `http://127.0.0.1:4174/` 的两个独立标签页，创建/加入同一房间后点击“加入语音”。该独立测试服务器使用合成静音音轨，不申请真实麦克风；顶部 QA 控件可模拟探索、会议与结算，显示 WebRTC 接收字节数。测试控件和状态修改路由不属于生产服务器，也不会进入静态构建。真实双设备麦克风音质与跨网络 TURN 连通性仍需部署后验收。

刷新页面会用当前标签页保存的会话重新连接，保留角色。断线超过 2 分钟后移出房间/淘汰；主动离开对局也会淘汰。完整邀请链接只含房间码，不含重连凭证。

## 本地验证

`npm ci` 然后 `npm start`，打开 http://127.0.0.1:4173/ 。本地自动连接相同地址的 WebSocket 服务。

`npm test` 包括真实 WebSocket 的 8 客户端联机测试。`npm run build` 验证 Vercel 静态产物。联机大厅仍提供“先玩单人练习”。

## 当前版本的边界

- 这是可部署的朋友房间版本，还未在你的 Vercel/Render 账号上发布；本地测试不能代替公网网络测试。
- 房间存在内存中，服务器重启、重新部署或休眠会清空房间，玩家需重新创建。Render 免费实例可能冷启动，首次连接需要等待后重试。官方说明：https://render.com/docs/free
- 移动由服务器校验并同步，浏览器做插值；跨地区高延迟仍可能影响手感。
- 无账号、排位、永久战绩或房间持久化；不要用于竞技比赛或大规模公开运营。小游戏答案验证防止误操作，但不等同于完整反作弊。
- Vercel 不运行 WebSocket 服务器：https://vercel.com/docs/limits 。Render 支持 WebSocket：https://render.com/docs/websocket 。
