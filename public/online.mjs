export function createOnline({onState,onStatus,onLocal,onCombat=()=>{}}){
 ...
 async function connect(action){
  ...
  try{
   ...
   ws=new WebSocket(url);const active=ws;const timeout=setTimeout(()=>{if(active.readyState!==1){active.close();say('连接超时，请检查服务器地址。');}},10000);
   ...
   ws.onclose=()=>{clearTimeout(timeout);connecting=false;if(manual)return;say('连接断开，正在尝试重连…');if(session){clearTimeout(retry);retry=setTimeout(()=>connect({type:'resume',...session}),1500);}else say('连接失败，请检查房间服务器是否启动。');};
   ws.onerror=()=>say('无法连接服务器，请检查部署地址与允许的网页域名。');
  }catch(e){connecting=false;say(e.message);}
 }
 function leave(){manual=true;clearTimeout(retry);send({type:'leave'});ws?.close();session=null;sessionStorage.removeItem('shadow-room');location.href=location.pathname;}
 ...
 el('local-mode').onclick=()=>{manual=true;ws?.close();dialog.close();onLocal();};
 ...
 return {get connected(){return ws?.readyState===1;},sendAction:action=>send({type:'action',action}),input(input,sprint){if(performance.now()-lastInput<50)return;lastInput=performance.now();const len=Math.max(1,Math.hypot(input.x,input.z));send({type:'input',x:input.x/len,z:input.z/len,sprint});},leave};
}
